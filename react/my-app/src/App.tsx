import { useState } from 'react';
import './App.css'
import { NavLink } from 'react-router';

function App() {

  var [pixBrCode, setPixBrCode] = useState("");
  var [pixQrCode, setPixQrCode] = useState("");

  const handleSetGlobals = (event: any) => {
    const globals = event.detail?.globals;
    console.log("Receiving Globals: ", globals);
    setPixBrCode((globals?.toolOutput?.pixBrCode ?? ""));
    setPixQrCode((globals?.toolOutput?.pixQrCode ?? ""));
    console.log(pixBrCode, pixQrCode);
  };

  window.addEventListener("openai:set_globals", handleSetGlobals, {
    passive: true,
  });

  if (window.openai) {
    pixBrCode = (window.openai?.toolOutput?.pixBrCode ?? "");
    pixQrCode = (window.openai?.toolOutput?.pixQrCode ?? "");
  } else {
    pixBrCode = "00020126450014BR.GOV.BCB.PIX0123gustavolive22@gmail.com52040000530398654031005802BR5918GUSTAVO DE O S SIL6009Sao Paulo62070503***6304045B";
    pixQrCode = "data:image/png;base64,";
  }

  if (pixBrCode) {

    return (
      <>
        <h1>Gerar Pagamento Pix</h1>
        <p className="pix-result">
          Seu código pix:{pixBrCode}
        </p>
        <img src={pixQrCode} />
        <button>
          Gerar Novo Pix
        </button>
        <NavLink to="/pix-list">Pagamentos Recentes</NavLink>
        <p>Esse aplicativo não é um mediador de transações e não salva dados pessoais.
          Verifique a consistência dos dados antes de fazer o pagamento no seu banco de preferência</p>
      </>
    )
  } else {
    return (
      <>
        <h1>Gerar Pagamento Pix</h1>
        <p>Carregando...</p>
      </>
    )
  }

}

export default App
