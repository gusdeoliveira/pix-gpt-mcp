...
## Architecture
- **MCP server (`server.js`)** - Boots `@modelcontextprotocol/sdk` with `StreamableHTTPServerTransport`, registers the `pix-widget` HTML resource, and wires one tool:
  - `generate_code`: validates user input with `zod`, infers `key_type` when omitted, formats merchant data via `steplix-emv-qrcps`, produces QR images with `qrcode`, caches the Pix payload, and returns structured JSON (`pixBrCode`, `pixQrCode`) plus textual status messages.
- **Widget surface (`public/index.html`)** - Delivered as `text/html;profile=mcp-app` so ChatGPT renders Pix cards inline. It binds to the host globals defined in `react/my-app/src/types/openai.d.ts` (`window.openai.callTool`, `requestDisplayMode`, `setWidgetState`, etc.) matching the Apps SDK UI contract.
- **React widget source (`react/my-app`)** - Source toolkit for iterating on widget behavior; run widget build and sync to refresh `public/index.html`.
...
- `npm run build:widget` - build the React widget (`react/my-app/dist/index.html`) and copy it into `public/index.html`.
- `npm start` - run `npm run build:widget` and then start the MCP server with `node --watch server.js`, exposing `/mcp` and a health check at `/`.
...
- Manual smoke tests: `curl http://localhost:8787/` should return the docs page HTML; invoking `generate_code` via the inspector should immediately render the Pix widget with the QR card.
